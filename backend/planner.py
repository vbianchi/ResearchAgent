...
3.  If no tool is directly needed for a step (e.g., the LLM itself will perform the reasoning, summarization, or content generation), use "None" for `tool_to_use`. #
    -   For such "No Tool" steps that involve the LLM generating specific text output (e.g., a poem, a summary, a specific format of data like JSON or Markdown):
        ...
        -   The `expected_outcome` for these generative "No Tool" steps MUST clearly state that the *actual generated content itself* is the outcome. #
            For example:
            - "The text of a short poem about stars." #
            (NOT "A poem is generated and available")
            - "A concise summary of the key findings from the search results." #
            (NOT "A summary is created")
            - "A JSON object listing the extracted entities." #
            (NOT "Entities are extracted and available in JSON")
    -   If a "No Tool" step involves complex, multi-part generation or intricate formatting (e.g., a detailed Markdown table from unstructured data), consider breaking it into two "No Tool" steps:
        1.  An initial step to generate the core data/elements. #
            `expected_outcome`: "Intermediate data/elements for X are generated." #
        2.  A subsequent step to format this intermediate data. #
            `expected_outcome`: "The [data from step 1] formatted as a Markdown table." #
4.  Provide brief `tool_input_instructions` highlighting key parameters or data the tool might need if a tool is used. #
    This is NOT the full tool input, but guidance for the agent when it forms the tool input later. #
    For "None" tool steps, this can be a brief note on what information the LLM should focus on if not obvious from the description. #
...
async def generate_plan(
    ...
    session_data_entry: Optional[Dict[str, Any]] = None, # For LLM override, though not directly used yet
    llm_instance: Optional[BaseChatModel] = None # <<< NEW PARAMETER
) -> Tuple[Optional[str], Optional[List[Dict[str, Any]]]]:
    """
    Generates a multi-step plan based on the user query using an LLM.
    Accepts an optional LLM instance, otherwise fetches its own based on settings.
    """
    logger.info(f"Planner: Generating plan for user query: {user_query[:100]}...")

    planner_llm_to_use: BaseChatModel
    if llm_instance:
        planner_llm_to_use = llm_instance
        logger.info(f"Planner: Using provided LLM instance: {type(llm_instance)}")
    else:
        # Fallback to fetching LLM based on settings if no instance is provided
        # Determine LLM provider and model from session_data_entry or settings
        planner_llm_id_override = session_data_entry.get("session_planner_llm_id") if session_data_entry else None
        ...
        if planner_llm_id_override:
            try:
                ...
                if override_provider and override_model_name: # Basic check
                    ...
                    logger.info(f"Planner: Using session override LLM for fallback: {provider}::{model_name}")
                else:
                    logger.warning(f"Planner: Invalid session LLM ID format '{planner_llm_id_override}'. Using system default for Planner fallback.")
            except ValueError:
                logger.warning(f"Planner: Could not parse session LLM ID '{planner_llm_id_override}'. Using system default for Planner fallback.")
        try:
            planner_llm_to_use = get_llm(
                settings,
                provider=provider,
                model_name=model_name,
                requested_for_role="Planner"
            )
            logger.info(f"Planner: Using LLM (fallback or default): {provider}::{model_name}") #
    ...
    chain = prompt_template | planner_llm_to_use | parser

    try:
        logger.debug(f"Planner prompt input variables: {prompt_template.input_variables}")
        # The chain.ainvoke should implicitly pick up RunnableConfig if llm_instance was created with it
        # or if this whole generate_plan is run within a LangGraph node that has config.
        ...
        if isinstance(planned_result_dict, AgentPlan):
            ...
            try:
                raw_output_chain = prompt_template | planner_llm_to_use | StrOutputParser()
        ...
        for i, step_model in enumerate(agent_plan.steps):
            step_dict = step_model.dict() # Convert Pydantic model to dict
            step_dict['step_id'] = i + 1 # Ensure step_id is correctly 1-indexed
        ...
        try:
            error_chain = prompt_template | planner_llm_to_use | StrOutputParser()
...
if __name__ == '__main__':
    async def test_planner_poem_generation(): #
        ...
        # Test with llm_instance=None to use default fetching logic for testing
        summary, plan = await generate_plan(query_poem, tools_summary, llm_instance=None)
